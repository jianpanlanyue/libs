...
namespace easysql
{
	typedef based::exception sql_error;

	#define THROW_SQL_EXCEPTION THROW_SQL_EXCEPTION0
	#define THROW_SQL_EXCEPTION0() throw(easysql::sql_error((int)get_error_no(),get_error_msg(),__FILE__,__LINE__))
	#define THROW_SQL_EXCEPTION2(ERROR_NO,ERROR_STR) throw(easysql::sql_error(ERROR_NO,ERROR_STR,__FILE__,__LINE__))
	...
	public:
		struct mysql_lib_init
		{
			mysql_lib_init()
			{
				...
				ENSURE(mysql_library_init(0,NULL,NULL) == 0).tips("mysql lib init fail").error(-1);
...
// 			}
		...
		{
			ENSURE(my_sql_ != NULL).tips("be sure successfully connect to database first").error(-1);
		...
		{
			ENSURE(mysql_real_connect(my_sql_, host, user, passwd, database, port, NULL, client_flag) != NULL).tips(get_error_msg()).error(get_error_no());
		}
		...
		unsigned long long query(const char* sql_str, const Args&... args)
		{
			...
			ENSURE(mysql_real_query(my_sql_, formated_sql.c_str(), formated_sql.size()) == 0)(formated_sql).tips(get_error_msg()).error(get_error_no());
		...
		unsigned long long next_result()
		{
			...
			if (res_ == NULL)
			{
				...
				ENSURE(error_no == 0).tips(get_error_msg()).error(error_no);	//之前的sql语句产生结果集，但store result 发生错误
		...
		{
			ENSURE(res_ != NULL).tips("be sure to successfully store result first").error(-1);
			...
			ENSURE(field_length != NULL).tips(get_error_msg()).error(get_error_no());
	...
	private:
		...
		{
			type_conv_.to_type(dest, row[field_index], field_len_array[field_index]);
		}
		...
		void format_sql_str(std::string& sql_str, int last_read_pos, const Arg& arg)
		{
			...
			ENSURE(pos != sql_str.npos)(sql_str).tips("placeholders' count in sql str not compatiable with real parameters").error(-1);
			...
			ENSURE(sql_str.find("?", pos) == sql_str.npos)(sql_str).tips("placeholders' count in sql str not compatiable with real parameters").error(-1);
		}
		...
		CRITICAL_SECTION& get_init_mutex()
		{
